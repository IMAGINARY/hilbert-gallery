  ...
  stepWithTimer() {
    if (this.isPlaying && Object.keys(this.delays).length) {
      ...
      Object.entries(this.delays).forEach(([stationId, delay]) => {
        if (delay <= 0.1) {
          ViewerConn.send(stationId, this.playlist[stationId][this.heads[stationId]]);
          this.delays[stationId] =
            this.playlist[stationId][this.heads[stationId]].args.duration * 1000;
